...
namespace {
  // Helper to log WebView2 controller mode to a file on Windows.
  void writeWebView2Log(const std::string& line, bool trunc)
  {
    try {
      ...
      if (s_logPath.empty() || trunc) {
        ...
        if (GetModuleFileNameW(nullptr, exeBuf, 32768) != 0) {
          path = std::filesystem::path(exeBuf).parent_path() / L"webview2_controller_path.txt";
        ...
        if (path.empty()) {
          ...
          if (GetTempPathW(MAX_PATH, tempPath) != 0) {
            path = std::filesystem::path(tempPath) / L"webview2_controller_path.txt";
          }
        }
        if (!path.empty()) {
          s_logPath = path;
      ...
      if (!path.empty()) {
        ...
        if (f) {
          if (trunc) {
            f << "file_location: " << path.string() << std::endl;
          }
          f << line << std::endl;
          f.flush();
        }
      ...
      // Swallow all logging errors – never crash app because of logging.
    }
  }
}
    ...
    auto callback = [parentWindow, willBeSurface, completionHandler, initialSettings](HRESULT result, wil::com_ptr<ICoreWebView2Environment> env) -> HRESULT
      {
        ...
        }
        {
          const std::string line1 = "[WebView2] willBeSurface=" + std::string(willBeSurface ? "true" : "false")
            ...
            + " hasEnv10=" + std::string(webViewEnv10 ? "true" : "false");
#ifndef NDEBUG
          debugLog(line1);
#endif
        ...
        if (willBeSurface && (webViewEnv10 || webViewEnv3)) {
          ...
          const std::string line3 = "[WebView2] Fallback to Controller (CreateCoreWebView2Controller) - WebView2 will create child HWND";
#ifndef NDEBUG
          debugLog(line3);
#endif
